...
/**
 * Every journey scene must exist in the art contract, because `SceneArt` throws on an unknown key
 * and the surface each section declares is read from there. Checking it here means a typo fails at
 * the first import rather than part-way through rendering the page.
 */
for (const key of JOURNEY_SCENES) {
  if (!(key in SCENES)) {
    throw new Error(
      `Journey scene "${key}" is not declared in src/lib/art/contract.mjs. ` +
...
export function sceneCopy(
  ...
  if (!found) {
    throw new Error(
      `No copy for journey scene "${key}". Add it under "scenes:" in ` +
